...
import { createSpriteFramesFromImg, generateSpriteFramesTres, generateFrameTextures } from './src/godot/sprite-frames.js';
    ...
    switch (command) {
        ...
        case 'pvf-list': {
            ...
            try {
                ...
                // 显示前 20 个文件
                const limit = values.limit ? parseInt(values.limit as string) : 20;
                ...
                for (const file of files) {
                    ...
                    console.log(`    Chunk: ${file.entry.chunkIndex}, Offset: ${file.entry.dataOffset}, Size: ${file.entry.dataSize}, Type: ${file.entry.dataType}`);
                    count++;
                }
            } catch (error) {
        ...
        case 'pvf-decode': {
            ...
            try {
                ...
                if (entry.dataType !== 1) {
                    console.error(`文件类型不是脚本 (Type=1): ${pvfPath} (Type=${entry.dataType})`);
                    process.exit(1);
                }
        ...
        case 'npk': {
            ...
            try {
                ...
                // 显示前 10 个文件
                const limit = values.limit ? parseInt(values.limit as string) : 10;
                const filter = values.filter as string | undefined;
        ...
        case 'npk-extract': {
            const npkIndex = values.index ? parseInt(values.index as string) : -1;
            if (npkIndex < 0) {
            ...
            try {
                ...
                // 写入文件
                const outputPath = values.output as string || entry.name.split('/').pop() || 'output';
        ...
        case 'img': {
            ...
            try {
                ...
                // 显示帧信息
                const limit = values.limit ? parseInt(values.limit as string) : 5;
                console.log(`\n帧列表 (显示前 ${Math.min(limit, img.frames.length)} 个):`);
                for (let i = 0; i < Math.min(limit, img.frames.length); i++) {
                    const frame = img.frames[i];
                    console.log(`  帧 ${i}: ${frame.width}x${frame.height} @ (${frame.offset.x}, ${frame.offset.y})`);
                    console.log(`    大小: ${frame.dataSize} 字节, 压缩: ${frame.compression}`);
                }
            } catch (error) {
        ...
        case 'img-export': {
            const frameIndex = values.index ? parseInt(values.index as string) : 0;
            ...
            try {
                ...
                // 导出指定帧或所有帧
                const framesToExport = frameIndex >= 0 ? [img.frames[frameIndex]] : img.frames;
        ...
        case 'godot-sprite': {
            ...
            try {
                ...
                // 创建 SpriteFrames
                const baseName = target.split('/').pop()?.replace(/\.[^.]+$/, '') || 'sprite';
                ...
                for (const texture of textures) {
                    const pngData = exportFrameToPng(img.frames[textures.indexOf(texture)]);
                    if (pngData) {
                ...
                // 生成场景文件
                const sceneContent = generateSimpleSpriteScene(`${baseName}.tres`, animName);
