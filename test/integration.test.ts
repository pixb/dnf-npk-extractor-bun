...
import { resolveString } from '../src/pvf/reader-s4a21.js';

// 测试文件路径 (需要根据实际情况调整)
const TEST_PVF_PATH = 'C:\\game\\86jpA21次元彼端\\S4A21_CN\\S4A21_CN\\script.pvf';
const TEST_NPK_DIR = 'C:\\game\\86jpA21次元彼端\\S4A21_CN\\S4A21_CN\\ImagePacks2';
...
describe('NPK 格式', () => {
    ...
    it('应该正确解析 NPK 头部', async () => {
        ...
        // 查找第一个 NPK 文件
        const files = fs.readdirSync(TEST_NPK_DIR).filter((f: string) => f.endsWith('.NPK') || f.endsWith('.npk'));
        if (files.length === 0) {
