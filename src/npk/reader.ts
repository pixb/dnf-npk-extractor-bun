...
import type { NpkArchive, NpkFileEntry, NpkHeader } from './types.js';
 ...
 * 获取 NPK 文件数据
 */
export function getNpkFileData(archive: NpkArchive, fileIndex: number): Buffer | null {
    ...
    return Buffer.from(archive.data.subarray(entry.offset, entry.offset + entry.size));
}

/**
 * 根据名称查找 NPK 文件
 */
export function findNpkFile(archive: NpkArchive, fileName: string): NpkFileEntry | undefined {
 ...
 * 列出 NPK 文件
 */
export function listNpkFiles(archive: NpkArchive, filter?: string): NpkFileEntry[] {
    if (!filter) {
        return archive.files;
    }

    const lowerFilter = filter.toLowerCase();
    return archive.files.filter(f => f.name.toLowerCase().includes(lowerFilter));
}
