...
export class PvfArchiveS4A21 {
	...
	private parse(): void {
		...
		if (this.header.signature !== MAGIC_SIGNATURE) {
			console.warn(
				`Invalid signature: 0x${this.header.signature.toString(16)}`,
			);
		}
		...
		// 6. 解析字符串表
		this.parseNameTable(
			data.subarray(nameOffset, nameOffset + this.header.nameTableSize),
		);
	}
	...
	private parseNameTable(nameTableData: Buffer): void {
		...
		// 解析 sTrA
		const strAResult = this.decryptStringBuffer(
			nameTableData,
			idx,
			"sTrA",
			STR_A_XOR,
		...
		if (strAResult.nextIdx + 8 <= nameTableData.length) {
			const strWResult = this.decryptStringBuffer(
				nameTableData,
				strAResult.nextIdx,
				"sTrW",
				STR_W_XOR,
	 ...
	 * 解密字符串缓冲区
	 */
	private decryptStringBuffer(
		data: Buffer,
		index: number,
		key: string,
		xorConst: number,
	): { buffer: Buffer; nextIdx: number } {
		...
		const _cnt2 = data.readUInt32LE(index + 4);
	 ...
	 * 根据路径查找并获取文件数据
	 */
	getFileByPath(relativePath: string): {
		name: string;
		path: string;
		data: Buffer;
		entry: PvfFileItemRaw;
	} | null {
